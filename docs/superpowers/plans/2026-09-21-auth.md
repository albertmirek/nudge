...
- [ ] **Step 6: Seed** — in `server/src/database/seed.ts` change the upsert to
      `{ id: DEV_USER_ID, name: 'Sandra', email: 'dev@nudge.local', timezone: 'Europe/Prague' }` (the verified flag and password are set in Task 8 once hashing exists). Update the comment above `DEV_USER_ID` to `/** Fixed id so Bruno and the seed agree on the dev user. */`.
